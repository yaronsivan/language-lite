...
export class AgentOrchestrator {
  ...
  mapLevelToCEFR(level) {
    const levelMapping = {
      'Beginner': 'A2',      // Changed from A1 to A2 (more practical for real beginners)
      'Intermediate': 'B1',  // Keep B1 (good intermediate level)
      'Advanced': 'B2'       // Changed from C1 to B2 (more realistic for "advanced" users)
    };
...
Return your response in JSON format:
{
  ...
  buildVocabularyPrompt({ adaptedText, targetLanguage, proficiencyLevel, motherTongue }) {
    // Get language-specific cognate examples
    const cognateExamples = this.getCognateExamples(targetLanguage, motherTongue);
...
CRITICAL VOCABULARY SELECTION RULES:
- Identify 5-8 key vocabulary words appropriate for ${proficiencyLevel} level
- PRIORITIZE words that are genuinely challenging for ${motherTongue} speakers
- STRICTLY AVOID obvious cognates and international words that ${motherTongue} speakers already understand
${cognateExamples}
- REJECT any word that looks/sounds similar to ${motherTongue} (even if spelling differs slightly)
- Focus on function words, connectors, and culturally-specific terms
- Choose words that represent the essence and uniqueness of ${targetLanguage}
- Avoid overly basic words that ${proficiencyLevel} learners should already know
- Select words that enhance comprehension and cultural understanding

EXAMPLES of GOOD vocabulary choices for ${targetLanguage}→${motherTongue}:
- Function words (quindi=therefore, invece=instead, però=however, allora=then)
- Verb forms unique to the language (vedevano=they saw, svolgevano=took place)
- Cultural/temporal concepts (abitanti=inhabitants, onore=honor)
- Prepositions and particles that don't exist in ${motherTongue}
- Common verbs with unique usage patterns

EXAMPLES of BAD choices to REJECT:
- Words ending in -zione (obvious Latin cognates)
- Words that look like English words (teatro, popolare, etc.)
- International academic/cultural terms
- Any word a ${motherTongue} speaker would immediately recognize
...
Return your response in JSON format:
{
  ...
  getCognateExamples(targetLanguage, motherTongue) {
    // Language-specific cognate awareness
    const cognateMap = {
      'Italian': {
        'English': {
          avoid: ['problema/problem', 'difficile/difficult', 'importante/important', 'interessante/interesting', 'possibile/possible', 'famiglia/family', 'persona/person', 'momento/moment', 'situazione/situation', 'informazione/information', 'teatro/theater', 'rappresentazioni/representations', 'spettacoli/spectacles', 'cerimonie/ceremonies', 'feste/festivals', 'popolare/popular', 'diffondere/diffuse', 'liberazione/liberation', 'democratica/democratic', 'culturale/cultural', 'politica/politics', 'particolare/particular', 'religioso/religious', 'sociale/social', 'naturale/natural'],
          examples: 'AVOID obvious cognates like: problema, teatro, rappresentazioni, spettacoli, cerimonie, feste, popolare, diffondere, democratica, culturale, religioso (these are too similar to English)'
        }
      },
      'Spanish': {
        'English': {
          avoid: ['problema/problem', 'importante/important', 'familia/family', 'momento/moment', 'persona/person', 'situación/situation', 'información/information'],
          examples: 'AVOID words like: problema, importante, familia, momento, información (obvious cognates)'
        }
      },
      'French': {
        'English': {
          avoid: ['problème/problem', 'important/important', 'famille/family', 'moment/moment', 'personne/person', 'situation/situation', 'information/information'],
          examples: 'AVOID words like: problème, important, famille, moment, information (obvious cognates)'
        }
      },
      'German': {
        'English': {
          avoid: ['Problem/problem', 'Familie/family', 'Moment/moment', 'Person/person', 'Situation/situation', 'Information/information'],
          examples: 'AVOID words like: Problem, Familie, Moment, Information (obvious cognates)'
        }
      }
    };

    const languageMap = cognateMap[targetLanguage];
    if (languageMap && languageMap[motherTongue]) {
      return `- ${languageMap[motherTongue].examples}`;
    }
    
    return '- AVOID obvious international words and cognates';
  }
  ...
  async callOpenAI(prompt, agentType) {
    ...
    const sanitizeJson = (str) => {
      return str
        ...
        // Fix common JSON string issues within values
        .replace(/(".*?")/g, (match) => {
          return match
            .replace(/\n/g, '\\n')
            .replace(/\r/g, '\\r')
            .replace(/\t/g, '\\t')
            .replace(/\\/g, '\\\\')
            .replace(/"/g, (m, offset, string) => {
              // Don't escape quotes that are already properly escaped or are the opening/closing quotes
              if (offset > 0 && string[offset - 1] === '\\') return m;
              if (offset === 0 || offset === string.length - 1) return m;
              return '\\"';
            });
        });
    };
    ...
    try {
      ...
      try {
        ...
        console.log(`OpenAI ${agentType} agent response parsed after sanitization`);
        return result;
      } catch (secondError) {
        try {
          // If that fails, try to extract JSON from the response (sometimes AI adds text around JSON)
          const jsonMatch = content.match(/\{.*\}/s);
          if (jsonMatch) {
            const extractedJson = sanitizeJson(jsonMatch[0]);
            const result = JSON.parse(extractedJson);
            console.log(`OpenAI ${agentType} agent response parsed after extraction and sanitization`);
            return result;
          }
          ...
          console.error('Parse error:', thirdError.message);
          throw new Error(`Invalid JSON response from AI agent (${agentType}): ${thirdError.message}`);
        }
      }
    }
  }
