...
export class AgentOrchestrator {
  ...
  async executeAdaptationPhase(input) {
    ...
    const languageRules = this.linguisticRules.languages[targetLanguage];
    const levelRules = languageRules?.levels?.[proficiencyLevel];
    const universalRules = this.linguisticRules.universal_principles;

    if (!languageRules || !levelRules) {
      throw new Error(`No rules found for ${targetLanguage} at ${proficiencyLevel} level`);
    }
  ...
  async executeReviewPhase(input, revisionCycle = 0) {
    const maxRevisions = this.config.workflows.text_adaptation.phases.find(p => p.name === "Pedagogical Review").max_revision_cycles;
  ...
  buildAdaptationPrompt({ originalText, targetLanguage, proficiencyLevel, motherTongue, languageRules, levelRules, universalRules, targetWordCount }) {
...
- Maintain the same number of paragraphs and paragraph structure
...
${this.formatUniversalRulesForPrompt(universalRules, proficiencyLevel)}
...
Return your response in JSON format:
{
  ...
  formatUniversalRulesForPrompt(universalRules, level) {
    ...
    return `
- Maximum sentence length: ${universalRules.sentence_complexity[level].max_clauses} clauses
- Vocabulary frequency: ${levelRules.description}
- Keep sentences ${universalRules.sentence_complexity[level].complexity}
`;
  }
